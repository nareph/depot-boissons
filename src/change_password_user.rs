// src/change_password_user.rs - Alternative Arc approach
...
pub fn show(user_id: String) -> AppResult<bool> {
    log::info!(
    ...
    // Wrap user_id in Arc for shared ownership
    ...
    ui.on_save_clicked(move |old_password, new_password| {
        if let Some(ui) = ui_handle.upgrade() {
            ...
            thread::spawn(move || {
                ...
                let user_check_result = match queries::get_user_by_id(&user_id_for_thread) { 
                    ...
                    Err(e) => {
                        log::error!("Impossible de trouver l'utilisateur {} pour vérifier le mot de passe: {}", user_id_for_thread, e);
                ...
                let final_result: Result<(), String> = match user_check_result {
                    Ok(()) => match bcrypt::hash(&new_password, bcrypt::DEFAULT_COST) {
                        Ok(hash) => {
                            ...
                            queries::update_user_password(&user_id_for_thread, &hash)
                                .map_err(|e| {
                ...
                let user_id_for_log = user_id_for_thread.clone();

                let _ = slint::invoke_from_event_loop(move || {
                    if let Some(ui) = thread_ui_handle.upgrade() {
                        match final_result {
                            Ok(()) => {
                                log::info!("Mot de passe changé avec succès par l'utilisateur ID {}.", user_id_for_log);
